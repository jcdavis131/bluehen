...
import { siteHref, SiteSubnav, PageHeader } from "@synthaembed/ui-fleet";
...
export const metadata = {
  title: "Operations Center — Blue Hen RE",
  description: "Operator control plane · jcamd.com",
};

export default async function ControlPage() {
  const surface = getSiteCircuit("control");
  const nav = getSiteNav("control");
  const sites = listSites().filter((s) => s.role !== "fleet-agent");
  const [online, fleet] = await Promise.all([getHealth(), getFleetStatus()]);
  ...
  return (
    <>
      <PageHeader
        ...
        title={surface?.stop ?? "Operations Center"}
        lead={
          <>
            {RE.relay} command center for {BRAND.operatingLoop}.{" "}
